...
export const WECHAT_SPEC_DOC_TEXT = `=======================================================
   微信开放平台 • 表情开放平台全套审核物料规范对照表
=======================================================
...
1. 01_表情主图_240x240_GIF/
   - 格式：GIF
   - 尺寸：严格 240 × 240 像素
   - 体积：每张必须小于 500KB（本工具生成文件均在 150KB ~ 350KB 间）
...
export async function exportWeChatPackageZip(
  ...
  // 1. 01_表情主图_240x240_GIF/
  const stickersFolder = rootFolder.folder('01_表情主图_240x240_GIF');
  if (stickersFolder) {
  ...
  rootFolder.file('使用说明与微信审核规范对照表.txt', WECHAT_SPEC_DOC_TEXT);
  ...
  a.href = downloadUrl;
  a.download = `${packageName}_${Date.now()}.zip`;
